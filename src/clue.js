...
function selectRandom(cardStack) {    // cardStack -----> pila de cartes
    // if (cardStack === 0){
    //     return null;
    // }
    // let randSelec = cardStack[Math.floor(Math.random()*cardStack.length)];
    // console.log(randSelec)
    // return randSelec;
    let randIndex = Math.floor(Math.random() * cardStack.length);
    let randElement = cardStack[randIndex];
    return randElement; 
...
function pickMystery() {
    ...
    crimeScene.weapon = weaponRandom;
    crimeScene.room = roomRandom;
